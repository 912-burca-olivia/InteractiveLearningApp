...
export class FileBrowserComponent implements OnInit {
  ...
  onFileDelete(fileView: any, e: Event) {
    if (e)
      ...
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover the file " + fileView.name,
        icon: "warning",
        dangerMode: true,
      })
      .then((willDelete) => {
  ...
  onFilter(e: Event) {
    if (this.filesCopy.length > 0) {
      if (this.filter.length > 0) {
        let filtered = this.filesCopy.filter((file: { name: string; }) => file.name.toLowerCase().indexOf(this.filter.toLowerCase()) != -1);
        this.files = filtered;
      }
      else
